...
# 느낀 점
- 이제야 조금씩 파이선, ipynb 사용 방법을 알아 가는 것 같다. gpt에게 선생님의 채점 ipynb파일과 내가 해온 과제 진행을 파일별로 전달 하면서 보여주니까 어디서부터 잘못됐는지, 채점은 어떻게 이루어 지는지 알려줘서 간단한 코딩만 하면 되는 것을 이해하게 해준다. 
- 전에는 대충 ipynb에다가 내 py 코드를 복붙 하면서 체점하는 줄 알았다.. 이젠 굳이 그럴 필요 없음을 이해했다. 
