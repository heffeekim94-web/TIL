...
복소수(Complex Number)는 실수(Real Number)와 허수(Imaginary Number)를 모두 포함하는 더 큰 개념의 수 체계입니다.

기본적으로 다음과 같은 형태로 나타냅니다:


$$a + bi$$

* **$a$ (실수부):** 우리가 평소에 쓰는 일반적인 숫자(예: 3, -1.5, 0 등)입니다.
* **$b$ (허수부):** 실수 $b$에다 허수단위 $i$를 곱한 것입니다.
* **$i$ (아이, Imaginary unit):** 제곱했을 때 `-1`이 되는 상상의 숫자입니다. ($\sqrt{-1} = i$, 즉 $i^2 = -1$)

### 왜 복소수가 필요할까요?

일상적인 계산이나 방정식에서는 실수만 있어도 충분하지만, 수학을 확장하다 보면 $x^2 + 1 = 0$처럼 "제곱해서 마이너스가 되는 수"를 반드시 다뤄야 하는 상황이 생깁니다. 실수의 세계에서는 이 방정식의 해를 구할 수 없기 때문에, 상상 속의 단위 $i$를 도입하여 수의 범위를 확장한 것이 바로 복소수입니다.

지금 공부하고 계신 프로그래밍 과목에서는 회전이나 3차원 공간 변환을 다룰 때 직접적으로 복소수를 쓰지는 않지만, 이와 아주 밀접한 개념인 '쿼터니언(Quaternion, 사원수)'이라는 확장된 수 체계를 나중에 다루게 됩니다. 쿼터니언은 복소수를 3차원으로 확장해서 3D 공간의 회전을 부드럽게 표현할 때 핵심적으로 쓰이는 도구입니다!
...
1. np.linalg.eig 으로 고유값/ 고유벡터 구하기
    - ✅고유벡터 
2. 고유 값으로 각도 구하기
    - ✅$\theta$ 
3. $R - R^T$ 공식을 사용하여 부호를 구하기 (회전 방향)
    - ✅회전 방향, 부호 +/- 
...
- check → ✅ ☑️ ✔️
  ...
  팁: 정확한 이름을 몰라도 face, food, animal, weather, music처럼 넓은 영어 단어
  로 찾아보면 됩니다. 시스템이나 앱에 따라 검색되는 이름과 후보는 조금씩 달라질
  수 있어요.
...
def axis_angle_from_matrix(R, atol: float = 1e-8):
    ...
    if R.shape != 3:
        raise ValueError("회전행렬의 shape은 3x3이어야 합니다. ")
